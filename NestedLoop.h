...
template <class T> class NestedLoop {
public:
    NestedLoop(vector<vector<T> > values);
    NestedLoop(const NestedLoop& orig);
    virtual ~NestedLoop();
    
    vector<vector<T> >  buildMatrix();
private:
...
template <class T> NestedLoop<T>::NestedLoop(vector<vector<T> > variables) {
    this->values = variables;
}

template <class T> NestedLoop<T>::NestedLoop(const NestedLoop& orig) {
}

template <class T> NestedLoop<T>::~NestedLoop() {
}
 ...
 */
template <class T> vector<vector<T> > NestedLoop<T>::buildMatrix() {
    typedef vector<T> assignment; //just a convenience typedef
